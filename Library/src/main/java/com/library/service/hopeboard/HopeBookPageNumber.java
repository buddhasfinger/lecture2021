...
@Component
public class HopeBookPageNumber {
	@Autowired
	...
	public Map<String, Object> PageNumber(int page, int limit, String category, String search) {
		...
		if (category == null || category.equals("")) {
			// 전체리스트 개수메소드
			listCount = board_mapper.listCount();
		} else if (category.equals("title")) {
			// title검색 개수메소드
			listCount = board_mapper.listCountTitle(search);
		} else if (category.equals("content")) {
			// Content검색 개수메소드
			listCount = board_mapper.listCountContent(search);
		} else if (category.equals("all")) {
			listCount = board_mapper.listCountSearchAll(search);
		}
